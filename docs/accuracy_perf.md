...
| `APP_NVINGEST_STOREIMAGES` | `true` | Set to `false` to skip storing extracted image assets for citations. Lite mode always disables image storage. | - Keeps multimodal citation assets available by default. <br/> | - Uses object-store capacity for extracted assets. <br/> |
